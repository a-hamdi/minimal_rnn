...
__global__ void min_lstm_extract_scan_params_kernel(const LinearLayer* d_linear_f,
    ...
    if (t < seq_length && h < hidden_size) {
        ...
        // Compute a_t = f_prime and b_t = i_prime * h_tilde
        a[t * hidden_size + h] = f_prime;
        b[t * hidden_size + h] = i_prime * h_tilde;
    }
}

// Helper for composing scan operations in parallel - same as in min_gru_cuda.cu
...
// Helper for applying scan operation in parallel - same as in min_gru_cuda.cu
...
// CUDA implementation of the parallel scan algorithm for MinLSTM
void min_lstm_parallel_scan_cuda(int seq_length, int batch_size, int hidden_size,
                               const float* a, const float* b, const float* h0,
                               float* h_out) {
    // This function is essentially identical to min_gru_parallel_scan_cuda
    // since the scan operation has the same form for both models
...
// Free device memory for MinLSTM cell
...
// Process a full sequence with MinLSTM using CUDA
